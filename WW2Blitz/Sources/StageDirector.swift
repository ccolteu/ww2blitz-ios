...
struct StageDirectors {
    static func create(waveScript: Int) -> StageDirector {
        ...
        case StageWaveKind.JUNGLE_RUINS:   return Stage6Director()
        case StageWaveKind.ASCENT_CANOPY:  return Stage7Director()
        case StageWaveKind.ORBIT_INTRO:    return Stage8Director()
        case StageWaveKind.FROZEN_FRONT:   return Stage4Director()
        case StageWaveKind.CORAL_ATOLL:    return Stage5Director()
...
class Stage6Director: StageDirector {
    private var flankGap: Float = FormationSpawner.S6_FLANK_SPACING
    private var weaveGap: Float = FormationSpawner.S6_WEAVE_SPACING
    ...
    func reset() {
        flankGap = FormationSpawner.S6_FLANK_SPACING; weaveGap = FormationSpawner.S6_WEAVE_SPACING
        weaveCount = 0; kamiSpawned = false; wallCount = 0; holdVSpawned = false; cruiserSpawned = false
    }
    func tick(dt: Float, elapsed: Float, enemies: EnemyPoolManager, w: Float, h: Float,
              boss: BossController, allowBoss: Bool, stageData: StageData, cue: DirectorCue) {
        if cue.bossCueFired { return }
        if elapsed >= FormationSpawner.S6_FLANK_START && elapsed <= FormationSpawner.S6_FLANK_END {
            flankGap += dt; var g = 0
            while flankGap >= FormationSpawner.S6_FLANK_SPACING && g < 2 {
                if enemies.countActive() >= FormationSpawner.MAX_ACTIVE { break }
                flankGap -= FormationSpawner.S6_FLANK_SPACING; g += 1
                let y = h * 0.40
                enemies.spawnEnemy(startX: -0.08*w, startY: y, velocityX: FormationSpawner.S6_FLANK_VX, velocityY: 0, enemyType: FormationSpawner.TYPE_DRONE)
                enemies.spawnEnemy(startX: 1.08*w, startY: y, velocityX: -FormationSpawner.S6_FLANK_VX, velocityY: 0, enemyType: FormationSpawner.TYPE_DRONE)
            }
        }
        if elapsed >= FormationSpawner.S6_WEAVE_START && elapsed <= FormationSpawner.S6_WEAVE_END && weaveCount < FormationSpawner.S6_WEAVE_PAIRS {
            weaveGap += dt; var g = 0
            while weaveCount < FormationSpawner.S6_WEAVE_PAIRS && weaveGap >= FormationSpawner.S6_WEAVE_SPACING && g < 2 {
                if enemies.countActive() >= FormationSpawner.MAX_ACTIVE { break }
                weaveGap -= FormationSpawner.S6_WEAVE_SPACING; g += 1; weaveCount += 1
                enemies.spawnEnemy(startX: 0.12*w, startY: -0.02*h, velocityX: 0, velocityY: FormationSpawner.S6_WEAVE_VY, enemyType: FormationSpawner.TYPE_DRONE, pattern: FormationSpawner.PATTERN_WEAVE)
                enemies.spawnEnemy(startX: 0.88*w, startY: -0.02*h, velocityX: 0, velocityY: FormationSpawner.S6_WEAVE_VY, enemyType: FormationSpawner.TYPE_DRONE, pattern: FormationSpawner.PATTERN_WEAVE)
            }
        }
        ...
        if !kamiSpawned && elapsed >= FormationSpawner.S6_KAMI_AT {
            ...
            enemies.spawnEnemy(startX: -0.06*w, startY: 0.22*h, velocityX: FormationSpawner.S6_FLANK_VX*0.90,
                               velocityY: FormationSpawner.S6_KAMI_VY*0.70, enemyType: FormationSpawner.TYPE_KAMIKAZE)
            enemies.spawnEnemy(startX: 1.06*w, startY: 0.22*h, velocityX: -FormationSpawner.S6_FLANK_VX*0.90,
                               velocityY: FormationSpawner.S6_KAMI_VY*0.70, enemyType: FormationSpawner.TYPE_KAMIKAZE)
        }
        if elapsed >= FormationSpawner.S6_WALL_START && elapsed <= FormationSpawner.S6_WALL_END && wallCount < FormationSpawner.S6_WALL_COUNT {
            let at = FormationSpawner.S6_WALL_START + Float(wallCount) * FormationSpawner.S6_WALL_SPACING
            if elapsed >= at {
                let lane: Float
                ...
                enemies.spawnEnemy(startX: lane*w, startY: -0.04*h, velocityX: 0, velocityY: FormationSpawner.S6_WALL_VY, enemyType: FormationSpawner.TYPE_DRONE)
                wallCount += 1
            }
        }
        if !holdVSpawned && elapsed >= FormationSpawner.S6_HOLD_V_AT {
            holdVSpawned = true
...
class Stage7Director: StageDirector {
    private var flankGap: Float = FormationSpawner.S7_FLANK_SPACING
    private var flankFromLeft = true
    ...
    func reset() {
        flankGap = FormationSpawner.S7_FLANK_SPACING; flankFromLeft = true
    ...
    func tick(dt: Float, elapsed: Float, enemies: EnemyPoolManager, w: Float, h: Float,
              boss: BossController, allowBoss: Bool, stageData: StageData, cue: DirectorCue) {
        if cue.bossCueFired { return }
        if elapsed <= FormationSpawner.S7_FLANK_END {
            flankGap += dt; var g = 0
            while flankGap >= FormationSpawner.S7_FLANK_SPACING && g < 2 {
                flankGap -= FormationSpawner.S7_FLANK_SPACING; g += 1
                FormationSpawner.spawnS7FlankCascade(enemies: enemies, w: w, h: h, fromLeft: flankFromLeft)
                flankFromLeft = !flankFromLeft
            }
        }
        if !kamiVSpawned && elapsed >= FormationSpawner.S7_KAMI_V_AT {
            kamiVSpawned = true
            FormationSpawner.spawnS7CenterKamiV(enemies: enemies, w: w, h: h)
        }
        if !heavyLeftSpawned && elapsed >= FormationSpawner.S7_HEAVY_LEFT_AT {
            ...
            enemies.spawnEnemy(startX: 0.20*w, startY: -0.10*h, velocityX: 0, velocityY: FormationSpawner.HEAVY_VY,
                               ...
                               health: FormationSpawner.S7_HEAVY_HP)
        }
        if !heavyRightSpawned && elapsed >= FormationSpawner.S7_HEAVY_RIGHT_AT {
            heavyRightSpawned = true
            enemies.spawnEnemy(startX: 0.80*w, startY: -0.10*h, velocityX: 0, velocityY: FormationSpawner.HEAVY_VY,
                               enemyType: FormationSpawner.TYPE_HEAVY, pattern: FormationSpawner.PATTERN_V_HOLD,
                               health: FormationSpawner.S7_HEAVY_HP)
        }
        if !wagonsSpawned && elapsed >= FormationSpawner.S7_WAGONS_AT {
            wagonsSpawned = true
            let vy = FormationSpawner.HEAVY_VY * 2.2
            enemies.spawnEnemy(startX: 0.22*w, startY: -0.06*h, velocityX: 0, velocityY: vy,
                               enemyType: FormationSpawner.TYPE_HEAVY, pattern: FormationSpawner.PATTERN_V_HOLD,
                               health: FormationSpawner.S7_WAGON_HP, isWagon: true)
            enemies.spawnEnemy(startX: 0.78*w, startY: -0.06*h, velocityX: 0, velocityY: vy,
                               enemyType: FormationSpawner.TYPE_HEAVY, pattern: FormationSpawner.PATTERN_V_HOLD,
                               health: FormationSpawner.S7_WAGON_HP, isWagon: true)
        }
        if elapsed >= FormationSpawner.S7_DRIZZLE_START && elapsed <= FormationSpawner.S7_DRIZZLE_END {
            drizzleGap += dt; var g = 0
            while drizzleGap >= FormationSpawner.S7_DRIZZLE_SPACING && g < 2 {
                drizzleGap -= FormationSpawner.S7_DRIZZLE_SPACING; g += 1
                drizzleSeed = drizzleSeed &* 1664525 &+ 1013904223
                let u = Float((drizzleSeed >> 8) & 0xFFFFFF) / 16_777_215
                let xFrac: Float = 0.08 + u * 0.84
                enemies.spawnEnemy(startX: xFrac*w, startY: -0.04*h, velocityX: 0, velocityY: FormationSpawner.S7_DRIZZLE_VY,
                                   enemyType: FormationSpawner.TYPE_INTERCEPTOR, pattern: FormationSpawner.PATTERN_WEAVE)
            }
        }
        if !powerWaveSpawned && elapsed >= FormationSpawner.S7_POWER_WAVE_AT {
            powerWaveSpawned = true
            enemies.spawnEnemy(startX: 0.25*w, startY: -0.05*h, velocityX: 0, velocityY: FormationSpawner.S7_POWER_VY,
                               enemyType: FormationSpawner.TYPE_DRONE, isRedShipAnchor: true)
            enemies.spawnEnemy(startX: 0.50*w, startY: -0.05*h, velocityX: 0, velocityY: FormationSpawner.S7_POWER_VY,
                               enemyType: FormationSpawner.TYPE_DRONE, isRedShipAnchor: true)
            enemies.spawnEnemy(startX: 0.75*w, startY: -0.05*h, velocityX: 0, velocityY: FormationSpawner.S7_POWER_VY,
                               enemyType: FormationSpawner.TYPE_DRONE, isRedShipAnchor: true)
        }
        if !kamiWallSpawned && elapsed >= FormationSpawner.S7_KAMI_WALL_AT {
            kamiWallSpawned = true
            var n = 0
            while n < 6 {
                let xFrac = (Float(n) + 0.5) / 6
                enemies.spawnEnemy(startX: xFrac*w, startY: -0.06*h, velocityX: 0, velocityY: FormationSpawner.S7_KAMI_VY,
                                   enemyType: FormationSpawner.TYPE_KAMIKAZE)
                n += 1
            }
        }
        if elapsed >= FormationSpawner.S7_SCROLL_DECAY_AT {
            let u = min(max((elapsed - FormationSpawner.S7_SCROLL_DECAY_AT) / FormationSpawner.S7_SCROLL_DECAY_SPAN, 0), 1)
            stageData.scrollSpeedY = FormationSpawner.S7_SCROLL_START * (1 - u)
        }
...
// MARK: - Stage 8 (intro only, no-op director)
...
class Stage4Director: StageDirector {
    ...
    func tick(dt: Float, elapsed: Float, enemies: EnemyPoolManager, w: Float, h: Float,
              boss: BossController, allowBoss: Bool, stageData: StageData, cue: DirectorCue) {
        if cue.bossCueFired { return }
        if elapsed >= FormationSpawner.S4_FLURRY_START && elapsed <= FormationSpawner.S4_FLURRY_END {
            flurryGap += dt; var g = 0
            while flurryGap >= FormationSpawner.S4_FLURRY_SPACING && g < 2 {
                if enemies.countActive() >= FormationSpawner.MAX_ACTIVE { break }
                flurryGap -= FormationSpawner.S4_FLURRY_SPACING; g += 1
                enemies.spawnEnemy(startX: 0.18*w, startY: -0.04*h, velocityX: 0, velocityY: FormationSpawner.S4_FLURRY_VY, enemyType: FormationSpawner.TYPE_DRONE, pattern: FormationSpawner.PATTERN_WEAVE)
                enemies.spawnEnemy(startX: 0.50*w, startY: -0.08*h, velocityX: 0, velocityY: FormationSpawner.S4_FLURRY_VY*0.92, enemyType: FormationSpawner.TYPE_DRONE, pattern: FormationSpawner.PATTERN_WEAVE)
        ...
        if !wallSpawned && elapsed >= FormationSpawner.S4_WALL_AT {
            ...
            enemies.spawnEnemy(startX: 0.36*w, startY: -0.10*h, velocityX: 0, velocityY: FormationSpawner.S4_FLURRY_VY, enemyType: FormationSpawner.TYPE_DRONE)
            enemies.spawnEnemy(startX: 0.64*w, startY: -0.10*h, velocityX: 0, velocityY: FormationSpawner.S4_FLURRY_VY, enemyType: FormationSpawner.TYPE_DRONE)
            enemies.spawnEnemy(startX: 0.88*w, startY: -0.06*h, velocityX: 0, velocityY: FormationSpawner.S4_FLURRY_VY, enemyType: FormationSpawner.TYPE_DRONE)
        }
    }
}
...
class Stage5Director: StageDirector {
    ...
    func tick(dt: Float, elapsed: Float, enemies: EnemyPoolManager, w: Float, h: Float,
              boss: BossController, allowBoss: Bool, stageData: StageData, cue: DirectorCue) {
        if cue.bossCueFired { return }
        if elapsed >= FormationSpawner.S5_REEF_START && elapsed <= FormationSpawner.S5_REEF_END {
            reefGap += dt; var g = 0
            while reefGap >= FormationSpawner.S5_REEF_SPACING && g < 2 {
                if enemies.countActive() >= FormationSpawner.MAX_ACTIVE { break }
                reefGap -= FormationSpawner.S5_REEF_SPACING; g += 1
                enemies.spawnEnemy(startX: 0.22*w, startY: -0.05*h, velocityX: 0, velocityY: FormationSpawner.S5_REEF_VY, enemyType: FormationSpawner.TYPE_DRONE, pattern: FormationSpawner.PATTERN_WEAVE)
        ...
        if !wallSpawned && elapsed >= FormationSpawner.S5_WALL_AT {
            ...
            enemies.spawnEnemy(startX: 0.38*w, startY: -0.10*h, velocityX: 0, velocityY: FormationSpawner.S5_REEF_VY*1.15, enemyType: FormationSpawner.TYPE_DRONE)
            enemies.spawnEnemy(startX: 0.62*w, startY: -0.10*h, velocityX: 0, velocityY: FormationSpawner.S5_REEF_VY*1.15, enemyType: FormationSpawner.TYPE_DRONE)
            enemies.spawnEnemy(startX: 0.86*w, startY: -0.06*h, velocityX: 0, velocityY: FormationSpawner.S5_REEF_VY*1.15, enemyType: FormationSpawner.TYPE_DRONE)
        }
    }
}
